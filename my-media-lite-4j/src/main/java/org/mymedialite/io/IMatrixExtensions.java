...
public class IMatrixExtensions {
  ...
  /**
   * Write a BooleanMatrix to a PrintWriter object.
   * @param writer a PrintWriter
   * @param matrix the BooleanMatrix to write out
   */
  public static void writeSparseMatrix(PrintWriter writer, IBooleanMatrix matrix) {
    writer.println(matrix.numberOfRows());
    for(int i = 0 ; i < matrix.numberOfRows(); i++) {
      writer.print(i);
      IntList row = matrix.getEntriesByRow(i);
      for(int attributeId : row) writer.print(" " + attributeId);
      writer.println();
    }
  }
  
  /**
   * Read a SparseBooleanMatrix from a BufferedReader object.
   * @param reader the BufferedReader object to read from
   * @return a matrix of doubles
   */
  public static SparseBooleanMatrix readSparseBooleanMatrix(BufferedReader reader) throws IOException {  
    ...
    for(int i=0; i < rowCount; i++) {
      ...
      for(int j=1; j < ids.length; j++) {
        int attributeId =  Integer.parseInt(ids[j]);
        matrix.set(itemId, attributeId, true);
      }
    }
    return matrix;
  }
  ...
  static public IMatrix<Integer> readIntegerMatrix(BufferedReader reader, IMatrix<Integer> example_matrix) throws IOException {
    ...
    System.out.println("numberOfRows = " + matrix.numberOfRows() + " numberOfColumns = " + matrix.numberOfColumns());
    return matrix;
  }
