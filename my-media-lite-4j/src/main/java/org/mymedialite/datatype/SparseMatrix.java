...
public class SparseMatrix<T> implements IMatrix<T> {
  ...
  protected List<Int2ObjectMap<T>> row_list = new ArrayList<Int2ObjectMap<T>>();
  ...
  public SparseMatrix(int num_rows, int num_cols, T d) {
    for (int i = 0; i < num_rows; i++) {
      ...
      row_list.add(new Int2ObjectArrayMap<T>());
    }
   ...
   * @param x the row ID
   */
  public Map<Integer, T> get(int x) {
    if (x >= row_list.size())
  ...
  public void set(int x, int y, T value) {
    if (x >= row_list.size())
      for (int i = row_list.size(); i <= x; i++) row_list.add(new Int2ObjectArrayMap<T>());
   ...
   * .
   */
  public HashMap<Integer, Map<Integer, T>> nonEmptyRows() {
    HashMap<Integer, Map<Integer, T>> return_list = new HashMap<Integer, Map<Integer, T>>();
    for(int i=0; i < row_list.size(); i++) {
      Map<Integer, T> row = get(i);
      if(row.size() > 0)
  ...
  public List<Pair<Integer, Integer>> nonEmptyEntryIDs() {
    ...
    for (Map.Entry<Integer, Map<Integer, T>> id_row : nonEmptyRows().entrySet())
  ...
  public int numberOfNonEmptyEntries() {
    int counter = 0;
    for (Map<Integer, T> row : row_list)
      counter += row.size();
    return counter;
  }
