...
public class Matrix<T> implements IMatrix<T>  {
   ...
   * Do nothing if num_rows is less than dim1.
   * @param num_rows the minimum number of rows
   * @param d the default value for elements in the new rows
   */
  public void addRows(int num_rows, T d) {
    int dim1 = this.dim1;
    addRows(num_rows);
    for(int i = dim1; i < num_rows; i++) {
      System.out.println("i = " + i);
      setRowToOneValue(i, d);
    }
  }
