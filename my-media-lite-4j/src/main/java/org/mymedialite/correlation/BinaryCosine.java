    ...
    for (int x = 0; x < numEntities; x++)
      for (int y = 0; y < x; y++) {
        if(entityData.numEntriesByRow(x) == 0 || entityData.numEntriesByRow(y) == 0) {
          ...
          set(x, y, (float)(overlap.get(x, y) / Math.sqrt((double)entityData.numEntriesByRow(x) * entityData.numEntriesByRow(y))));
        }
      }
  }
    ...
    for (int x = 0; x < numEntities; x++)
      for (int y = 0; y < x; y++) {
        if(entity_data.numEntriesByRow(x) == 0 || entity_data.numEntriesByRow(y) == 0) {
          ...
          set(x, y, (float) (overlap.get(x, y) / Math.sqrt((double)entity_data.numEntriesByRow(x) * entity_data.numEntriesByRow(y))));
        }
      }
  }
  ...
  public static float computeCorrelation(HashSet<Integer> vectorI, HashSet<Integer> vectorJ) {
    ...
    for (int k : vectorJ)
      if (vectorI.contains(k))
        cntr++;
    return cntr / (float) Math.sqrt((double)vectorI.size() * vectorJ.size());
  }
}
