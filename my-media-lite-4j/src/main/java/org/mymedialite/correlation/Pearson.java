...
public class Pearson extends RatingCorrelationMatrix {
  ...
  public void computeCorrelations(IRatings ratings, EntityType entityType) {
    ...
    for (List<Integer> other_entity_ratings : ratings_by_other_entity) {               // user, list of rating indices by user
      for (int i = 0; i < other_entity_ratings.size(); i++) {
        ...
        for (int j = i + 1; j < other_entity_ratings.size(); j++) {
          ...
          System.out.println();
          System.out.println("i: " + i + " j: " + j + " rating1: " + rating1 + " rating2: " + rating2);
...
//          System.out.println("freqs: " + freqs.get(x, y));
//          System.out.println("i_sums: " + i_sums.get(x, y));
//          System.out.println("j_sums: " + j_sums.get(x, y));
//          System.out.println("ij_sums: " + ij_sums.get(x, y));
//          System.out.println("ii_sums: " + ii_sums.get(x, y));
//          System.out.println("jj_sums: " + jj_sums.get(x, y));
    ...
    for (int i = 0; i < numEntities; i++) {
      for (int j = i + 1; j < numEntities; j++) {
        ...
        double first = n * ii_sums.get(i, j) - i_sums.get(i, j) * i_sums.get(i, j);
        double second = n * jj_sums.get(i, j) - j_sums.get(i, j) * j_sums.get(i, j);
        ...
        // double numerator = productSum * n - ( ( pSum * qSum ) );
        // double denominator = Math.Sqrt( ( pSumSq - ( pSum * pSum ) ) * ( qSumSq - ( qSum * qSum )) 
        
        
        double pmcc = numerator / denominator;
        // TODO restore shrinkage
        //this.set(i, j, (float) (pmcc * ((n - 1) / (n - 1 + shrinkage))));
        System.out.println("i: " + i + " j: " + j + " pmcc: " + pmcc);
        this.set(i, j, (float) (pmcc));
      }
    }
  }
